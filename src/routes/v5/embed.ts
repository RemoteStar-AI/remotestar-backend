...
async function processFile(
  ...
  try {
    ...
    ); // You may want to update this prompt to clarify file context
    ...
    const cfPrompt = updatedCulturalFitPrompt(
      "a file has been uploaded of that candidate use that to extract the data"
    );
    logger.info(
    ...
    const skPrompt = updatedSkillsPrompt(
      "a file has been uploaded of that candidate use that to extract the data"
    );
    ...
    try {
      ...
      (async () => {
        try {
          ...
          // Emit progress updates in order
...
resumeUploadRouter.get(
  ...
  (req: any, res: any) => {
    ...
    logger.info(
      `[ROUTE] Status check requested for processingId: ${processingId}`
    );
    logger.info(
      `[ROUTE] Status object for processingId ${processingId}: ${JSON.stringify(
        status
      )}`
    );
